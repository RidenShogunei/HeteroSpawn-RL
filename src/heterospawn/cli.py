...
def build_parser() -> argparse.ArgumentParser:
    ...
    wideseek_sft_smoke = subparsers.add_parser(
        "wideseek-sft-smoke",
        help="run one real-model shared-policy WideSeek SFT update and restore",
    )
    ...
    wideseek_sft_smoke.add_argument(
        ...
        type=int,
        dest="task_indices",
        help="zero-based training task index; repeat for multiple tasks (default: 0)",
    )
    wideseek_sft_smoke.add_argument(
        "--data-manifest",
        type=Path,
    ...
    wideseek_sft_smoke.add_argument("--device", default="cuda:0")
    wideseek_sft_smoke.add_argument(
        "--model-profile",
        choices=("qwen2.5-0.5b", "qwen3-4b"),
        default="qwen3-4b",
    )
    wideseek_sft_smoke.add_argument("--model-path", type=Path)
    wideseek_sft_smoke.add_argument(
        "--model-manifest",
        type=Path,
        help="trusted multi-file model manifest; required by the qwen3-4b profile",
    ...
    wideseek_sft_smoke.add_argument(
        "--artifact-dir",
        type=Path,
        default=Path("artifacts/wideseek-sft-smoke/checkpoints"),
    )
    wideseek_sft_smoke.add_argument(
        "--report",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "wideseek-sft-dry-run":
        ...
        return 0
    if args.command == "wideseek-sft-smoke":
        if args.model_path is None and not args.allow_model_download:
